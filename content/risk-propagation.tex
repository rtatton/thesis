...
The interface of an actor is defined by three types of messages: contacts, flush timeouts, and risk scores. As with \Cref{sec:synchronous}, contacts and risk scores have finite relevance such that the \define{time-to-live} (TTL) is defined as the remaining time of its relevance. In the context of asynchronous risk propagation, the reference time $\vRefTime$ is assumed to be the current time.
%
\begin{function}{\nRiskScoreTtl}[\vScore]
...
\end{function}
%
% State: actor - exposure score; contacts - send threshold
%
...
% Finite relevance and caching
% Message optimizations - sender-side batching with flush timeouts
% Perspective - a contact is a stateful filter, a channel, and a proxy of the receiving actor that represents the physical individual with which the individual represented by this actor came into close proximity
...
The \cHandleContact{} operation defines how an actor processes a contact message.
%
