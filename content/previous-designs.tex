...
\begin{function}{\nRiskPropagationActor}[\vGraph_k, \vScores_k]
  \ForEach{$\vVertex_i \in \vGraph_k$}
    \State $r_i \assign (\vScore_i^0, \vTime_i^0) = \max \vScores_i$
    \ForEach{$\vVertex_j \in \vNeighbors_i$}
      \State $\vMessage_{ij} \assign \Call{Compute-Message}{\vVertex_j, \vScores_i}$
  ...
  \While{termination condition is not satisfied}
    ...
    \ForEach{$\vVertex_\ell \in \vNeighbors_i \setminus \{\vVertex_j\}$}
      \State $\vMessage_{i\ell} \assign \Call{Compute-Message}{\vVertex_\ell, \vMessage_{ji}}$
      \If{$\vScore_{i\ell} \geq \pSendCoefficient \vScore_i^0$ \AND $\vTime_{i\ell} \leq \vTime_i^0$}
...
\begin{function}{\nComputeMessage}[\vVertex_\ell, \vMessage_{ji}]
  \State $(\vScore^\star, \vTime^\star) \assign \argmax \SetBuilder{\vScore^{\lambda(\vTime - \vTime_{ij})}}{(\vScore, \vTime) \in \vMessage_{ji}, \vTime < \vTime_{ij} + \pTimeBuffer}$
  \State \Return $(\pTransmissionRate\vScore^\star, \vTime^\star)$
\end{function}

%Compared to the approaches described in \Cref{ch:risk-propagation}:
%
%Similar to synchronous risk propagation:
%  - Offline – implies static topology
%
%Similar to asynchronous risk propagation:
%  - Uses send coefficient to terminate message passing
%  - Besides the initial message, each risk score is processed individually
%  
%Unique traits:
%  - Uses additional termination conditions from MWD approach (see preprint)
%  - Computing the first risk score accounts for the recency of the previously computed risk score – no effect after the first message
%  - Temporal filtering based on the initial risk score
%  - Message reachability is also defined with temporal filtering

% Recognized that the state maintained by the monitor can be distributed amongst actors that represent individuals – this avoids the bottleneck
% First exposure to the actor model and temporal networks
% Message passing amongst actors that represent contact subnetworks
% Asynchronous, but still offline since it assumes an "initial" risk score – send threshold/coefficient maintained for all contacts of a user, rather than individual contacts
% Use of Ray library is likely the reason for poor runtime performance and limited concurrency
