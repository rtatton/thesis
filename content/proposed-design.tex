...
\begin{itemize}
  \item $\aActorExposure$: the current exposure score of the individual. This attribute is either a symptom score, a risk score received from another actor, or the null risk score (see \cNullRiskScore).
...
\begin{function}{\nUpdateExposureScore}[\vActor, \vScore]
  ...
  \ElsIf{$\cRiskScoreTtl[\aActorExposure] \leq 0$}
    \State $\aActorExposure \assign \cMaxRiskScore[\vActor, \infty]$
  \EndIf
\end{function}
...
\begin{function}{\nUpdateSendThreshold}[\vActor, \vContact]
  \If{$\aContactThresholdValue > 0 \AND \cRiskScoreTtl[\aContactThreshold] \leq 0$}\label{step:should-update-threshold}
    \State $\vScore \assign \cMaxRiskScore[\vActor, \aContactTime + \pTimeBuffer]$\label{step:get-max-older-than}
    \State $\aNewScoreValue \assign \pTransmissionRate \cdot \aScoreValue$
    \State $\cSetSendThreshold[\vContact, \vNewScore]$
  \EndIf
\end{function}
