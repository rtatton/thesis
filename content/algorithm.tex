...
\newcommand{\vMaxExposureScore}{r}
\newcommand{\vRiskScores}[2]{\vExpScores[#1]^{(#2)}}
\NewDocumentCommand{\vExposureScores}{om}{
  \IfValueTF{#1}
    {\vMaxExposureScore_{#1}^{(#2)}}
    {\mathbf{\vMaxExposureScore}^{(#2)}}
}
\newcommand{\dist}{d}
...
\end{enumerate}
%
To summarize, a factor message $\vFactorMsg{i}{j}{n}$ is the maximum relevant risk score in the variable message $\vVariableMsg{i}{j}{n}$ (or 0) that is scaled by the transmission rate $\pTransmissionRate$.
...
%
\begin{equation*}
  \vMessageSet_k = \setBuilder{\pTransmissionRate \vScore_t}{\vScore_t \in \vVariableMsg{i}{j}{n}, t < t_k + \pTimeBuffer}.
\end{equation*}
%
...
To detect convergence, the normed difference between the current and previous exposure scores is compared to the threshold $\pDiff \in \reals$. Note that $\vExposureScores{n}$ is the vector of exposure scores in the \indexed{n}{iteration} such that $\vExposureScores[i]{n}$ is \indexed{i}{component} of $\vExposureScores{n}$. The $\ell^1$ and $\ell^\infty$ norms are sensible choices for detecting convergence. \citet{Ayday2021} use the $\ell^1$ norm, which ensures that an individual's exposure score changed by at most $\pDiff$ after the penultimate iteration.
...
\begin{function}[H]{\nRiskPropagation}[\vScores, \vContacts]
  ...
  \ForEach{$\vVariable[i] \in \vVariables$}
    ...
    \State $\vExposureScores[i]{n - 1} \assign \max \vRiskScores{i}{n - 1}$
    \State $\vExposureScores[i]{n} \assign \infty$
  \EndFor
  \While{$\| \vExposureScores{n} - \vExposureScores{n - 1} \| > \pDiff$}
    \ForEach{$\{\vVariable[i], \vFactor[ij]\} \in \vEdges$}
      \State $\vVariableMsg{i}{j}{n} \assign \vRiskScores{i}{n - 1} \setminus \setBuilder{\vFactorMsg{j}{i}{k}}{k \in [1 \twodots n - 1]}$
    \EndFor
    \ForEach{$\{\vVariable[i], \vFactor[ij]\} \in \vEdges$}
      \State $\vFactorMsg{i}{j}{n} \assign \max \setBuilder{\pTransmissionRate \vScore_t}{\vScore_t \in \vVariableMsg{i}{j}{n}, t < t_{ij} + \pTimeBuffer}$
    \EndFor
    \ForEach{$\vVariable[i] \in \vVariables$}
      \State $\vRiskScores{i}{n} \assign \topK{\setBuilder{\vFactorMsg{j}{i}{n}}{\vFactor[ij] \in \vNeighbors_i}}$
    \EndFor
    \ForEach{$\vVariable[i] \in \vVariables$}
      \State $\vExposureScores[i]{n - 1} \assign \vExposureScores[i]{n}$
      \State $\vExposureScores[i]{n} \assign \max \vRiskScores{i}{n}$
    \EndFor
    \State $n \assign n + 1$
  \EndWhile
...
While straightforward to specify, the description of risk propagation in \Cref{sec:synchronous}, is not a viable implementation for real-world application. \cRiskPropagation{} is an \define{offline algorithm} because it requires all individuals' recent contacts and risk scores to run. As \citet{Ayday2021} note, this is not privacy-preserving. An offline design is also suboptimal with respect computational efficiency and human safety. Specifically, most exposure scores may not change across invocations of \cRiskPropagation{}, which implies communication overhead and computational redundancy. As a mitigation, \cRiskPropagation{} can only be executed once or twice daily, which was originally proposed by \citet{Ayday2020}. However, this causes substantial delay in reporting to individuals their new exposure scores; and in the face of a pandemic, timely information is essential for individual and collective health.

\citet{Ayday2021} propose distributing the computation amongst the devices or ``personal clouds'' of individuals such that each computing element is responsible for maintaining the individual's variable node and neighboring factor nodes. However, their proposal does not address the aspects of synchronization, termination, and communication efficiency; all of which are necessary to consider in a distributed setting.

Key observations:

- Message passing can be asynchronous...but how do ensure termination?
...
\subsection{Actor Behavior}\label{sec:behavior}
...
%
\begin{function}{Handle-Contact}[\vActor, \vContact]
  \If{$\cContactTtl[c] > 0$}
...
\end{function}
%
\begin{function}{Handle-Flush-Timeout}[\vActor]
...
%
\begin{function}{Handle-Risk-Score}[\vActor, \vScore]
...
\begin{function}{\nUpdateExposureScore}[\vActor, \vScore]
  ...
  \ElsIf{$\cRiskScoreTtl[\aActorScore] \leq 0$}
    \State $\aActorScore \assign \cCacheMax[\aActorScores]$
  \EndIf
\end{function}
%
...
%
\begin{itemize}
    \item The \define{influence set} $\vInfluenceSet(v)$ of node $v$ is the set of nodes that $v$ can reach by a time-respecting path.
    \item The \define{source set} $\vSourceSet(v)$ of node $v$ is the set of nodes that can reach $v$ by a time-respecting path.
...
Node $v$ is \define{message reachable} from node $u$ if there exists a shortest path such that $\vMsgReach(u, v) > 0$. The \define{message reachability} of node $u$ is the maximum message reachability from node $u$:
%
...
\end{align*}
%
Let $\mathbf{M}$ be the \define{message-reachability matrix} of the temporal network $G$ such that nodes are enumerated $1, 2, \ldots, \card{\vVariables}$ and for each $\vMsg_{ij} \in \mathbf{M}$,
%
\begin{equation*}
	\vMsg_{ij} = 
	   \begin{cases}
...
\begin{equation}\label{eq:reach}
    \vMsgReach(u, v) = \sum_{(i, j) \in \vPathSet} [\pTransmissionRate^{i} \vScore_u > \pSendCoefficient \pTransmissionRate \vScore_{ij}] \cdot [t_u < t_{ij} + \pTimeBuffer]
\end{equation}
%
are the value and contact-time constraints in the \cShouldContactReceive[] operation (see \Cref{sec:behavior}), where $(\vScore_i, t_i)$ is the current exposure score for actor $i$ and $t_{ij}$ is the most recent contact time between actors $i$ and $j$.
...
%
\begin{equation*}
    \pTransmissionRate^{\vEstMsgReach(u, v)} \cdot \vScoreValue_u \leq \pSendCoefficient \cdot \vScoreValue_v,
\end{equation*}
%
...
\begin{equation*}
    ...
    \begin{cases} 
        ...
        \log_{\pTransmissionRate}\pSendCoefficient + \log_{\pTransmissionRate}\vScoreValue_v - \log_{\pTransmissionRate}\vScoreValue_u & \text{otherwise.}
    \end{cases}
\end{equation*}
%
...
\end{semilogyaxis}
\end{tikzpicture}
\caption[Estimated risk propagation message reachability]{Estimated risk propagation message reachability $\vEstMsgReach(u)$ for different values of the transmission rate $\pTransmissionRate$, the send tolerance $\pSendCoefficient$, and the initial magnitude $\vScoreValue(v) = \vInitScoreValue(v) / \pTransmissionRate$ of the destination user $v$ with respect to the initial magnitude $\vScore_u = \vInitScoreValue(u) / \pTransmissionRate$ of the source user $u$.}
\label{fig:reach}
\end{figure}
...
%\begin{tikzpicture}
%\begin{loglogaxis}[
%  xlabel={Estimated message reachability},
%  ylabel={Risk score},
%  width=0.7\textwidth,
%]
%  \foreach \n in {1,...,5} {
